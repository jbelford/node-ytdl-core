 ...
 * @returns {any}
 */
const findJSON = (source, varName, body, search) => {
  let startIdx = body.search(search);
  if (startIdx >= 0) {
  ...
  return parseJSON(source, varName, utils.cutAfterJSON(body, startIdx));
};
...
const getVideoInfoPage = async(id, options) => {
  ...
  url.searchParams.set('c', 'ANDROID');
  url.searchParams.set('cver', '16.24');
